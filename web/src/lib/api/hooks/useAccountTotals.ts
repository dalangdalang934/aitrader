    ...
    const schedule = () => {
      ...
      timeout = window.setTimeout(async () => {
        try {
          await mutate(
            async (prev) => {
              const fresh = await fetcher(baseKey) as AccountTotalsResponse;
              const existing = prev?.accountTotals ?? [];
              const incoming = fresh?.accountTotals ?? [];
              if (!incoming.length) return prev ?? { accountTotals: [] };
              ...
              return { accountTotals: Array.from(map.values()).sort(sortRows) };
            },
            { revalidate: false },
          );
        } finally {
