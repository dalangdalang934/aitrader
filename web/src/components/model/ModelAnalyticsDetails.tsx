...
export default function ModelAnalyticsDetails({
  ...
  const analyticsRow = map[modelId];
  const [open, setOpen] = useState(false);

  if (!analyticsRow || Object.keys(analyticsRow).length === 0) return null;

  const t = analyticsRow.overall_trades_overview_table || {};
  const w = analyticsRow.winners_losers_breakdown_table || {};
  const f = analyticsRow.fee_pnl_moves_breakdown_table || {};
  const s = analyticsRow.signals_breakdown_table || {};
  const inv = analyticsRow.invocation_breakdown_table || {};
  const ls = analyticsRow.longs_shorts_breakdown_table || {};
...
function percentMix(
  signals?: AnalyticsRow["signals_breakdown_table"],
) {
  const L = signals?.long_signal_pct;
  const S = signals?.short_signal_pct;
  const H = signals?.hold_signal_pct;
  const C = signals?.close_signal_pct;
  const parts: string[] = [];
