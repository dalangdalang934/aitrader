...
import { usePositions, type RawPositionRow } from "@/lib/api/hooks/usePositions";
...
export default function ModelStatsSummary({ modelId }: { modelId: string }) {
  ...
  const latest = useMemo(() => {
    ...
    for (let i = arr.length - 1; i >= 0; i--) {
      const r = arr[i] as AccountTotalsRow;
  ...
  const analyticsRow: AnalyticsRow | undefined = analytics[modelId];
  const fees = analyticsRow?.fee_pnl_moves_breakdown_table?.total_fees_paid;
  const biggestWin = analyticsRow?.fee_pnl_moves_breakdown_table?.biggest_net_gain;
  const biggestLoss = analyticsRow?.fee_pnl_moves_breakdown_table?.biggest_net_loss;
  const avgConf = analyticsRow?.signals_breakdown_table?.avg_confidence; // 0-1
  ...
  const avgLev = useMemo(() => {
    ...
    const fromOverall =
      analyticsRow?.overall_trades_overview_table?.avg_convo_leverage;
    if (typeof fromOverall === "number" && fromOverall > 0) return fromOverall;
    // 2) 其次 signals_breakdown_table.avg_leverage
    const fromSignals = analyticsRow?.signals_breakdown_table?.avg_leverage;
