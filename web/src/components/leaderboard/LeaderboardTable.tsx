...
type DerivedLeaderboardRow = LeaderboardRow & {
  equity: number;
  return_pct?: number;
  win_rate?: number;
  total_pnl?: number;
...
export default function LeaderboardTable({
        ...
        <table className="w-full text-left text-[11px]">
          ...
          <tbody style={{ color: "var(--foreground)" }}>
            {isLoading ? (
              ...
              data.map(
                (r: DerivedLeaderboardRow, idx: number) => (
                  <tr
                    key={r.id}
                  ...
                  >
                    ...
                    {mode === "advanced" ? (
                      <>
                        ...
                        <td className="py-1 pr-2 lg:pr-3 tabular-nums">
                          {renderWinRate(r.win_rate, r.num_trades)}
                        </td>
                        <td
                          className={clsx(
                            "py-1 pr-2 lg:pr-3 tabular-nums",
  ...
  function renderSharpe(id: string, value?: number) {
    ...
    const content = (
      <div className="space-y-1">
        ...
        <div className="opacity-80">{getSharpeHint()}</div>
      </div>
    );
    const muted = n < 3;
    return (
  ...
  function renderFees(id: string, fees?: number) {
    ...
    const avg = analyticsRow?.fee_pnl_moves_breakdown_table?.avg_taker_fee;
    const content = (
      <div className="space-y-1">
  ...
  function renderWinRate(rate?: number, trades?: number) {
    const wr = rate;
    const content = (
      <div className="space-y-1">
